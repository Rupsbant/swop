...
public class NormalOrderPlacer implements OrderPlacer {
    ...
    public int checkStock(int currentStock, int ordered, int maxStock) throws ArgumentConstraintException {
        if (!(currentStock >= 0)) {
            throw new ArgumentConstraintException("A stock can not hold a negative amount of items.");
        }
        if (!(ordered >= 0)) {
            throw new ArgumentConstraintException("A negative amount of items can not be ordered.");
        }
        if (!(maxStock > 0)) {
            throw new ArgumentConstraintException("The stock needs a capacity larger than 0.");
        }
        int currentSpaceAvailable = maxStock - currentStock;
        if (!(currentSpaceAvailable >= 0)) {
            throw new ArgumentConstraintException("The stock can not be filled over capacity.");
        }
